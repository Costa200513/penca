...
async function handleVerifyEmail(code) {
  ...
  try {
    ...
    verifyMessage.textContent =
      "Tu dirección de correo fue confirmada correctamente. Ya podés iniciar sesión en la penca.";
  } catch (error) {
    console.error(error);
    verifyTitle.textContent = "No se pudo verificar";
    verifyMessage.textContent =
      "El enlace puede estar vencido, incompleto o ya fue utilizado. Probá registrarte nuevamente o solicitá otro correo de verificación.";
  }
}
...
async function handleResetPassword(code) {
  ...
  try {
    ...
    console.error(error);
    resetEmailText.textContent =
      "El enlace de recuperación no es válido o ya venció.";
    showMessage(
      resetPasswordMessage,
      "Solicitá un nuevo enlace desde la página de recuperación de contraseña.",
      "error",
    );
    return;
  }
  ...
  resetPasswordForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      showMessage(
        resetPasswordMessage,
        "No se pudo cambiar la contraseña. Solicitá un nuevo enlace.",
        "error",
      );
    } finally {
