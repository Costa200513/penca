...
import { collection, doc, getDoc, getDocs, setDoc, updateDoc, writeBatch, serverTimestamp, query, orderBy, onSnapshot } from 'https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js';
...
function renderSoon(){
  ...
  renderTimer = setTimeout(() => { if (currentUser && userData) renderAll(); }, 80);
}
...
function championSelectionClosed(){ return Date.now() >= CHAMPION_DEADLINE.getTime(); }
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (!user.emailVerified) { alert('Debés verificar tu correo electrónico antes de ingresar. Revisá tu bandeja de entrada.'); await signOut(auth); window.location.href = 'login.html'; return; }
  currentUser = user;
...
function renderMatchCard(m){
  ...
  const resultHtml = m.status==='played' ? `<strong>${m.goalsA} - ${m.goalsB}</strong><span>Resultado</span>` : `<strong>${pr?`${pr.goalsA} - ${pr.goalsB}`:'-'}</strong><span>Tu pronóstico</span>`;
...
function specialtyRankingData(){
  ...
  rankingData().forEach(u => {
    ...
    if (!bySpecialty.has(key)) {
      bySpecialty.set(key, { username:key, specialty:key, fullName:key, points:0, aciertos:0, exact:0, partial:0, draw:0, penalties:0, incorrect:0, users:0 });
    }
  ...
  return [...bySpecialty.values()].sort((a,b)=>b.points-a.points || b.exact-a.exact).map((r,i)=>({...r, position:i+1}));
}

function renderRankingBlock(title, description, data, mode='user'){
  const top=data.slice(0,3); const rest=data.slice(3); const podium=[top[1],top[0],top[2]].filter(Boolean); const cls={1:'first',2:'second',3:'third'}, med={1:'🥇',2:'🥈',3:'🥉'};
  const label = (r) => mode==='specialty' ? esc(r.specialty) : '@'+esc(r.username);
  const sub = (r) => mode==='specialty' ? `${r.users} participantes` : `${esc(r.specialty||'-')} · ${esc(r.year||'-')}.º`;
  return `<div class="ranking-block"><div class="ranking-list-header"><div><h3>${title}</h3><p>${description}</p></div></div><div class="podium">${podium.map(r=>`<article class="podium-card ${cls[r.position]||''}"><div class="podium-medal">${med[r.position]||'🏅'}</div><h3 class="podium-username">${label(r)}</h3><p class="podium-realname">${sub(r)}</p><div class="podium-points">${r.points} pts</div><div class="podium-correct">${r.aciertos} aciertos</div><button class="details-btn podium-details-btn" onclick="toggleRankingDetails(this)">+</button><div class="ranking-details podium-details">${detailPills(r)}</div></article>`).join('')}</div><div class="ranking-list">${rest.map(r=>`<div class="ranking-item"><div class="ranking-row"><span class="position">${r.position}.º</span><span class="ranking-user"><strong>${label(r)}</strong><small>${sub(r)}</small></span><span>${r.aciertos} aciertos</span><span class="points">${r.points} pts</span><button class="details-btn" onclick="toggleRankingDetails(this)">+</button></div><div class="ranking-details">${detailPills(r)}</div></div>`).join('') || '<div class="empty-ranking">No hay más posiciones para mostrar.</div>'}</div></div>`;
}
...
function renderRanking(){
  ...
  $('ranking').innerHTML = `<h1>Ranking</h1><div class="underline"></div><p class="subtitle">El admin no participa del ranking.</p><div class="ranking-container">${renderRankingBlock('Ranking por usuario','Puntaje individual de cada participante.', usersRank, 'user')}${renderRankingBlock('Ranking por especialidad','Suma de puntos por especialidad.', specialtyRank, 'specialty')}</div>`;
}
      ...
      : `<form id="profileChampionForm" class="champion-form champion-form-clean">
        <label>Seleccionar campeón
          <select id="profileChampionId" required>
            ...
            ${realTeamsSorted().map(t=>`<option value="${esc(t.id)}">${esc(t.name)}</option>`).join('')}
          </select>
        </label>
...
function realTeamsOptions(selected=''){ return '<option value="">Seleccionar</option>'+realTeamsSorted().map(t=>`<option value="${t.id}" ${t.id===selected?'selected':''}>${esc(t.name)}</option>`).join(''); }
...
function teamOptions(selected){ return sortByName([...teams.values()]).map(t=>`<option value="${t.id}" ${t.id===selected?'selected':''}>${esc(t.name)}</option>`).join(''); }
async function saveResult(e){
  ...
  const goalsA=parseGoalValue(fd.get('goalsA')), goalsB=parseGoalValue(fd.get('goalsB'));
  if (goalsA === null || goalsB === null) return alert('Los goles deben ser números enteros entre 0 y 50.');
...
async function saveSchedule(e){
  ...
  if(!/^\d{2}:\d{2}$/.test(matchTime)) return alert('Seleccioná una hora válida.');
  const dateOnly = parseDateOnlyFromMatch(m);
  if(!dateOnly) return alert('Este partido no tiene día cargado. Revisá la base inicial.');
